...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Go through all the actions and check if their all of their preconditions are present in S(i-1) level
        # if present create a pgNode_a

        self.a_levels.append(set())

        # for action in self.all_actions:
        #     a_node = PgNode_a(action)
        #     pre_nodes = a_node.prenodes
        #     for s_node in self.s_levels[level]:
        #         if s_node in pre_nodes:
    ...
    def add_literal_level(self, level):
        ...
        # add new element(set) to the s_levels; create S(level)
        self.s_levels.append(set())

        # going through Ai-1 nodes
        for a_node in self.a_levels[level-1]:
            # PgNode_s nodes in effect nodes of a_nodes for list of parents for the next S level
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e1 in node_a1.effnodes:
            for e2 in node_a2.effnodes:
                # if both symbols are same, ie. same action and have opposite symbols, that means the effect of
                # both the actions negate each other
                if (e1.symbol == e2.symbol) and (e1.is_pos == (not e2.is_pos)):
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e1 in node_a1.effnodes:
            for p2 in node_a2.prenodes:
                # if both symbols are same, ie. they are the same action and have opposite symbols, that means the
                # effect of both the actions negate each other
                if (e1.symbol == p2.symbol) and (e1.is_pos == (not p2.is_pos)):
                    return True

        for p1 in node_a1.prenodes:
            for e2 in node_a2.effnodes:
                # if both symbols are same, ie. same action and have opposite symbols, that means the effect of
                # both the actions negate each other
                if (p1.symbol == e2.symbol) and (p1.is_pos == (not e2.is_pos)):
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for p1 in node_a1.parents:
            for p2 in node_a2.parents:
                # we don't check the preconditions of the as we did in other mutex checks because the parents might be
                # mutex because of the other mutex checks which might noe have same symbol and opposite is_pos
                if p1.is_mutex(p2):
                    return True
        return False
